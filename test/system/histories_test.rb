...
class Historiestest < ApplicationSystemTestCase
  ...
  test 'edit history when user logged in' do
    ...
    visit edit_child_history_path(alice_child.id, histories(:carol_history_hib_first).id, vaccination_id: vaccinations(:hib_1))
  ...
  test 'not edit history before user logged in' do
    setup_alice
    logout
    visit edit_child_history_path(children(:carol).id, histories(:carol_history_hib_first).id, vaccination_id: vaccinations(:hib_1))
  ...
  test 'not edit any child other than their own' do
    ...
    visit edit_child_history_path(bob_child.id, histories(:dave_history_hib_first).id, vaccination_id: vaccinations(:hib_1))
    assert_text '子どもの登録'
  end
  ...
  test 'validation history before today' do
    ...
    visit edit_child_history_path(carol.id, histories(:carol_history_hib_first).id, vaccination_id: vaccinations(:hib_1))
  ...
  test 'validation bigger than before history' do
    ...
    visit edit_child_history_path(carol.id, histories(:carol_history_rotavirus_third).id, vaccination_id: vaccinations(:rotavirus_3))
  ...
  test 'validation smaller than before history' do
    ...
    visit edit_child_history_path(carol.id, histories(:carol_history_rotavirus_second).id, vaccination_id: vaccinations(:rotavirus_2))
    fill_in '接種日', with: Date.current - 1.month + 1.day
    click_on '登録する'
